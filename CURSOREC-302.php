...
// Consulta para obtener los datos del curso con ID 302
$sql = "SELECT ID_CURSO, ID_UBICACION, CANTIDAD_ALUMNOS_CURSO, DIRECTOR_CURSO, ESTADO_CURSO FROM curso WHERE ID_CURSO = 302";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['CANTIDAD_ALUMNOS_CURSO'])) {
        ...
        $sqlUpdate = "UPDATE curso SET CANTIDAD_ALUMNOS_CURSO = ?, DIRECTOR_CURSO = ?, ESTADO_CURSO = ? WHERE ID_CURSO = 302";
        $stmt = $conexion->prepare($sqlUpdate);
        $stmt->bind_param("ssi", $cantidad, $director, $estado); // Quitar $id, ya que ID_CURSO es constante 302

        if ($stmt->execute()) {
            // Redirigir a CURSOREC-302.php después de la actualización exitosa
            header("Location: CURSOREC-302.php");
        ...
        if ($stmt->execute()) {
            ...
            // Redirigir a CURSOREC-302.php después de la inserción exitosa
            header("Location: CURSOREC-302.php");
            exit(); // Terminar el script después de la redirección
        } else {
        ...
        <article>
            ...
            <section>
                ...
                <dialog id="modal">
                    ...
                    <form method="POST" action="CURSOREC-302.php">
                        <input type="hidden" name="ID_CURSO" value="302">
