...
// Consulta para obtener los datos del curso con ID 802
$sql = "SELECT ID_CURSO, ID_UBICACION, CANTIDAD_ALUMNOS_CURSO, DIRECTOR_CURSO, ESTADO_CURSO FROM curso WHERE ID_CURSO = 802";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['CANTIDAD_ALUMNOS_CURSO'])) {
        ...
        $sqlUpdate = "UPDATE curso SET CANTIDAD_ALUMNOS_CURSO = ?, DIRECTOR_CURSO = ?, ESTADO_CURSO = ? WHERE ID_CURSO = 802";
        $stmt = $conexion->prepare($sqlUpdate);
        $stmt->bind_param("ssi", $cantidad, $director, $estado); // Quitar $id, ya que ID_CURSO es constante 802

        if ($stmt->execute()) {
            // Redirigir a CURSOREC-802.php después de la actualización exitosa
            header("Location: CURSOREC-802.php");
        ...
        if ($stmt->execute()) {
            ...
            // Redirigir a CURSOREC-802.php después de la inserción exitosa
            header("Location: CURSOREC-802.php");
            exit(); // Terminar el script después de la redirección
        } else {
        ...
        <article>
            ...
            <section>
                ...
                <dialog id="modal">
                    ...
                    <form method="POST" action="CURSOREC-802.php">
                        <input type="hidden" name="ID_CURSO" value="802">
