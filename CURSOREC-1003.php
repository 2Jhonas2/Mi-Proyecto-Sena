...
// Consulta para obtener los datos del curso con ID 1003
$sql = "SELECT ID_CURSO, ID_UBICACION, CANTIDAD_ALUMNOS_CURSO, DIRECTOR_CURSO, ESTADO_CURSO FROM curso WHERE ID_CURSO = 1003";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['CANTIDAD_ALUMNOS_CURSO'])) {
        ...
        $sqlUpdate = "UPDATE curso SET CANTIDAD_ALUMNOS_CURSO = ?, DIRECTOR_CURSO = ?, ESTADO_CURSO = ? WHERE ID_CURSO = 1003";
        $stmt = $conexion->prepare($sqlUpdate);
        $stmt->bind_param("ssi", $cantidad, $director, $estado); // Quitar $id, ya que ID_CURSO es constante 1003

        if ($stmt->execute()) {
            // Redirigir a CURSOREC-1003.php después de la actualización exitosa
            header("Location: CURSOREC-1003.php");
        ...
        if ($stmt->execute()) {
            ...
            // Redirigir a CURSOREC-1003.php después de la inserción exitosa
            header("Location: CURSOREC-1003.php");
            exit(); // Terminar el script después de la redirección
        } else {
        ...
        <article>
            ...
            <section>
                ...
                <dialog id="modal">
                    ...
                    <form method="POST" action="CURSOREC-1003.php">
                        <input type="hidden" name="ID_CURSO" value="1003">
